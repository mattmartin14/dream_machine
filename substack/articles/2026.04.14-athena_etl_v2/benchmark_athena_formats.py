...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--database",
        default=None,
        help="Legacy shorthand to use one database for both JSON and Parquet tables.",
    )
    parser.add_argument(
        "--json-database",
        default=None,
        help="Database containing the raw JSON table (default: db1_staging).",
    )
    parser.add_argument(
        "--parquet-database",
        default=None,
        help="Database containing the Parquet table (default: db1).",
    )
    ...
    if args.database:
        args.json_database = args.database
        args.parquet_database = args.database
    else:
        args.json_database = args.json_database or "db1_staging"
        args.parquet_database = args.parquet_database or "db1"
...
def build_queries(
    json_database: str,
    parquet_database: str,
    json_table: str,
    parquet_table: str,
) -> list[tuple[str, str, str]]:
    json_fqtn = f"{json_database}.{json_table}"
    parquet_fqtn = f"{parquet_database}.{parquet_table}"

    return [
        (
            "row_count",
            f"SELECT count(*) FROM {json_fqtn} WHERE order_id IS NOT NULL",
            f"SELECT count(*) FROM {parquet_fqtn}",
        ),
        (
            "channel_sales",
            (
...
def main() -> None:
    ...
    pairs = build_queries(
        json_database=args.json_database,
        parquet_database=args.parquet_database,
        json_table=args.json_table,
        parquet_table=args.parquet_table,
    )

    print("Running Athena benchmarks with:")
    print(f"  json database: {args.json_database}")
    print(f"  parquet database: {args.parquet_database}")
